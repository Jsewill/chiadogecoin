...
import chiadoge.server.ws_connection as ws
import dns.asyncresolver
from chiadoge.protocols import full_node_protocol, introducer_protocol
from chiadoge.protocols.protocol_message_types import ProtocolMessageTypes
from chiadoge.server.address_manager import AddressManager, ExtendedPeerInfo
from chiadoge.server.address_manager_store import AddressManagerStore
from chiadoge.server.outbound_message import NodeType, make_msg
from chiadoge.server.server import ChiadogeServer
from chiadoge.types.peer_info import PeerInfo, TimestampedPeerInfo
from chiadoge.util.hash import std_hash
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: ChiadogeServer = server
    ...
    async def on_connect(self, peer: ws.WSChiadogeConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSChiadogeConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSChiadogeConnection):
