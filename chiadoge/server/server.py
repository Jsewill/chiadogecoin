...
from chiadoge.protocols.shared_protocol import protocol_version
from chiadoge.server.introducer_peers import IntroducerPeers
from chiadoge.server.outbound_message import Message, NodeType
from chiadoge.server.ssl_context import private_ssl_paths, public_ssl_paths
from chiadoge.server.ws_connection import WSChiadogeConnection
from chiadoge.types.blockchain_format.sized_bytes import bytes32
from chiadoge.types.peer_info import PeerInfo
from chiadoge.util.errors import Err, ProtocolError
from chiadoge.util.ints import uint16
from chiadoge.util.network import is_localhost, is_in_network
...
class ChiadogeServer:
    def __init__(
        ...
        chiadoge_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSChiadogeConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSChiadogeConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.chiadoge_ca_crt_path, self.chiadoge_ca_key_path = chiadoge_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSChiadogeConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.chiadoge_ca_crt_path, self.chiadoge_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSChiadogeConnection] = None
        try:
            connection = WSChiadogeConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSChiadogeConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSChiadogeConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSChiadogeConnection, task_id):
    ...
    async def send_to_others(
        ...
        node_type: NodeType,
        origin_peer: WSChiadogeConnection,
    ):
    ...
    def get_outgoing_connections(self) -> List[WSChiadogeConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSChiadogeConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSChiadogeConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self) -> List[WSChiadogeConnection]:
        result = []
        for _, connection in self.all_connections.items():
            result.append(connection)
        return result
    ...
    def is_trusted_peer(self, peer: WSChiadogeConnection, trusted_peers: Dict) -> bool:
